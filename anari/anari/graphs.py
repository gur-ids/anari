...
def scatter_plot_toi_pts(plot_id, df):
    ...
    for i in df.Position.unique():
        ...
        sizeref = 4.0 * max(size) / (25.0**2)
        text = df_by_position.apply(
            lambda x: tv.name_salary(x["H-Ref Name"], x["Cap Hit"]), axis=1
        )
        ...
        traces.append(
            go.Scatter(
                ...
                name=next(
                    (
                        d.get("label")
                        for d in tv.position_label_values
                        if d.get("value") == i
                    ),
                    "Label not found",
                ),
...
def update_detailed_team_graphs(df, x_value, y_value, title):
    ...
    return dcc.Graph(
        figure={
            "data": traces,
            "layout": go.Layout(
                xaxis={"title": x_value, "type": "linear"},
                yaxis={"title": y_value, "type": "linear"},
                legend={"x": 0, "y": 1},
                hovermode="closest",
                title=title,
            ),
...
def regression_scatter(lr_data, category):
    ...
    layout = go.Layout(
        ...
        width=width,
        height=height,
        xaxis=go.layout.XAxis(tickmode="linear", dtick=dtick, title="Actual value"),
        yaxis=go.layout.YAxis(tickmode="linear", dtick=dtick, title="Predicted value"),
    )
    ...
    for i in df.Position.unique():
        ...
        traces.append(
            go.Scatter(
                ...
                text=df_by_position.apply(
                    lambda x: tv.name_salary_pts(
                        x["H-Ref Name"], x["Cap Hit"], x["forecast_PTS"]
                    ),
                    axis=1,
                ),
                hoverinfo="text",
